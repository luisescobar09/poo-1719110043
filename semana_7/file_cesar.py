class Cadena():#creamos una clase
  archivos = open("archivo.txt","r")#pedimos que abra el archivo con "open" y que lo lea con "r"
  print("La cadena es:")
  for line in archivos:#recorrerá cada caracter que tenga el txt
    print(line)#imprimimos la linea
  archivos.close()#cerramos el archivo con "close"
  cadena=line.lower().replace(",", "").replace(".", "").replace(" ", "").replace("á", "a").replace("é", "e").replace("í", "i").replace("ó", "o").replace("ú", "u")#lower convierte todas las letras a minusculas y replace cambia una vocal con acento a sin acento, las "," "." y espacios los elimina, para que al final la cadena termine junta
  ...
  def cifrar_descifrar(self): #creamos el método requerido
    if self.clave>26: #si es mayor la clave a 26cancelamos lo demas
      ...
      if self.decidir=="cifrar": #si decide cifrar
        ...
        print("La cadena descrifrada es:")
        for i in self.cadena: #creamos un FOR para separar cada caracter
          ascii=ord(i) #lo convertimos a codigo ascii
          ...
          print(final.upper())#imprimimos el resultado
